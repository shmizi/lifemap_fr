...
// lives in <SubgoalForm>, mounted fresh each open so its fields seed from props
// via useState initializers (no set-state-in-effect re-seed). Writes go through
// the store's addSubgoal / editSubgoal actions (the store owns `order`).
...
function SubgoalForm({
  ...
  const [targetDate, setTargetDate] = useState(subgoal?.targetDate ?? '')
  ...
  const [requiresConsistency, setRequiresConsistency] = useState(
    subgoal?.requiresConsistency ?? false,
  )
